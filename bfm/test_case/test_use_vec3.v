initial begin
   if($test$plusargs("test_use_vec3"))begin
      @(ev_load_mem);

      //config: TX data and RX data are valid in a while (depend on pause rate)
      `XMIT.setcfg_length(XMIT_TESTLEN);
      `XMIT.setcfg_pause(1);
      `XMIT.setcfg_pause_rate(17);

      `RCVR.setcfg_length(RCVR_TESTLEN);
      `RCVR.setcfg_pause(1);
      `RCVR.setcfg_pause_rate(13);
